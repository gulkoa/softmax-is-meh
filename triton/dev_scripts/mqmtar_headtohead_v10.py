...
V10 = V9 + --ift: exact IFT gradient for the dense Stieltjes arm via one
differentiable Newton step on the detached bisection root. Phase-1 result
motivating it: the FROZEN-scale asstjd arm oscillates too (0.70-0.91 band,
no consolidation) — the instability is in the mapping's gradient, not the
learnable scale. H4: the detached-lambda gradient's kappa*1_argmax term
jumps when the argmax flips; near-one-hot attention makes those flips
frequent -> oscillation. IFT is smooth everywhere (and is our flash
kernel's DEFAULT backward).
V9 — stability-study instrumentation (finding 2026-07-15: asstjd reaches
the transition doorstep then collapses; full-budget run died at NaN):
  --freeze-scale    : beta/gamma projections requires_grad=False — the
                      culprit-isolation ablation: if plain Stieltjes+NAPE
                      (constant init scale) trains stably, the LEARNABLE
                      scale is definitively the destabilizer
  --scale-lr-mult M : beta/gamma projections in their own optimizer param
                      group at lr*M (H2: stiff scale loss surface needs a
                      smaller step, not a bound)
  + per-500-step wandb logging of ||grad beta_proj||, ||grad gamma_proj||,
    beta max/mean (H1 signature: gradient spike BEFORE the loss collapse)
...
def stieltjes_dense_probs(scores, q_order, iters=50, eps=1e-20, ift=False):
    ...
    sum(w) = 1, then p = w / sum(w). By default lambda is used DETACHED
    (Jack's a19bb33 `stieltjes`: torch.where-blocked bisection +
    renormalization — the gradient form our kernel's normalize=True mode
    reproduces). The detached gradient carries a kappa*1_argmax term that
    JUMPS when the argmax flips — hypothesis H4 for the post-doorstep
    oscillation.

    ift=True appends ONE DIFFERENTIABLE Newton step at the detached root:
    lam = lam_det - f(lam_det)/f'(lam_det) with f built from live scores.
    At convergence f(lam_det) ~ 0 so the VALUE is unchanged, but autograd
    through the step yields exactly the implicit-function-theorem gradient
    dlam/ds_j = r_j / sum(r) — smooth everywhere (the gradient form of our
    kernel's DEFAULT mode). Masked entries (-1e9): (lam-s) ~ 1e9 -> w -> 0.
    """
    K = scores.shape[-1]
    with torch.no_grad():
    ...
    if ift:
        ...
        lam = lam - f_val / f_der          # differentiable Newton step
    ...
    def forward(self, x):
        ...
        if self.cfg.arm == "sdpa":
            ...
            p = stieltjes_dense_probs(scores, self.cfg.stj_q,
                                      ift=getattr(self.cfg, "ift", False))
            o = torch.einsum("bhst,bhtd->bhsd", p.to(v.dtype), v)
        else:
...
def main():
    ...
    ap.add_argument("--freeze-scale", action="store_true", dest="freeze_scale",
                    help="beta/gamma projections requires_grad=False "
                         "(culprit-isolation ablation)")
    ap.add_argument("--scale-lr-mult", type=float, default=1.0,
                    dest="scale_lr_mult",
                    help="beta/gamma projections in a separate param group "
                         "at lr*mult")
    ap.add_argument("--ift", action="store_true",
                    help="IFT gradient for asstjd: one differentiable "
                         "Newton step on the detached bisection root "
                         "(smooth dlam/ds = r/sum(r) instead of the "
                         "argmax-discontinuous detached form)")
    args = ap.parse_args()
    ...
    args.arm_label = (args.arm if args.arm not in ("asstj", "asstjd")
                     ...
                     ("-frz" if args.freeze_scale else "") + \
                     (f"-slr{args.scale_lr_mult:g}"
                      if args.scale_lr_mult != 1.0 else "") + \
                     ("-ift" if args.ift else "")
    ...
    scale_params = [p for name, p in model.named_parameters()
                    if "beta_proj" in name or "gamma_proj" in name]
    other_params = [p for name, p in model.named_parameters()
                    if "beta_proj" not in name and "gamma_proj" not in name]
    if args.freeze_scale:
        for p in scale_params:
            p.requires_grad_(False)
        print(f"scale params FROZEN ({sum(p.numel() for p in scale_params)})")
    if scale_params and not args.freeze_scale and args.scale_lr_mult != 1.0:
        opt = torch.optim.AdamW(
            [{"params": other_params},
             {"params": scale_params, "lr": args.lr * args.scale_lr_mult}],
            lr=args.lr, weight_decay=0.0, betas=(0.9, 0.99))
        print(f"scale params at lr*{args.scale_lr_mult:g}")
    else:
        opt = torch.optim.AdamW(
    ...
    for step in range(args.steps):
        ...
        loss.backward()
        if step % 500 == 0 and scale_params and not args.freeze_scale:
            # stability instrumentation BEFORE clipping: H1 (runaway)
            # predicts scale-grad/weight-norm growth before the collapse
            with torch.no_grad():
                gb = torch.stack([p.grad.norm() for p in scale_params
                                  if p.grad is not None]).norm().item()
                wb = torch.stack([p.norm()
                                  for p in scale_params]).norm().item()
                run.log({"step": step, "scale_grad_norm": gb,
                         "scale_weight_norm": wb})
