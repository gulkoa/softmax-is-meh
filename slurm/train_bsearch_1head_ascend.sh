#!/bin/bash
# 1-HEAD 1-LAYER toy model for BINARY_SEARCH — Ascend.
...
OUT_DIR="results/binary_search_1head_${TAG}_seq${TRAIN_SEQ}_d${N_EMBD}_nope_seed${SEED}_ascend"
mkdir -p "${OUT_DIR}"

echo "=== binary_search 1-head 1-layer ${TAG} seq=${TRAIN_SEQ} d=${N_EMBD} seed=${SEED} NoPE Ascend ==="
nvidia-smi

python -u nanogpt/train.py \
    --task binary_search --max-arr-len "${ARR}" \
    ${EXTRA_TRAIN} \
    --pos-enc none \
    --seq-len "${TRAIN_SEQ}" \
