#!/bin/bash
# 1-HEAD 1-LAYER toy model for BFS retrieval — Ascend.
...
OUT_DIR="results/bfs_1head_${TAG}_seq${TRAIN_SEQ}_d${N_EMBD}_nope_seed${SEED}_ascend"
mkdir -p "${OUT_DIR}"

echo "=== bfs 1-head 1-layer ${TAG} seq=${TRAIN_SEQ} d=${N_EMBD} seed=${SEED} NoPE Ascend ==="
nvidia-smi

python -u nanogpt/train.py \
    --task bfs --max-arr-len "${ARR}" \
    ${EXTRA_TRAIN} \
    --pos-enc none \
    --seq-len "${TRAIN_SEQ}" \
