#!/bin/bash
# 1-HEAD 1-LAYER toy model for MAX retrieval — Ascend v2 (adds SEED env var).
...
OUT_DIR="results/max_1head_${TAG}_seq${TRAIN_SEQ}_d${N_EMBD}_nope_seed${SEED}_ascend"
mkdir -p "${OUT_DIR}"

echo "=== max 1-head 1-layer ${TAG} seq=${TRAIN_SEQ} d=${N_EMBD} seed=${SEED} NoPE Ascend ==="
nvidia-smi
