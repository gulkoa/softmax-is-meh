#!/bin/bash
# Max retrieval: n_embd scan to find softmax-vs-stj crossover point.
...
echo "=== MAX embd-scan: ${TAG} n_embd=${NEMBD} n_head=${NHEAD} ==="
...
for ES in 128 256 512 1024 2048 4096 8192; do
    ...
    python nanogpt/eval_accuracy.py \
        ...
        --n-layer 1 --n-head ${NHEAD} --n-embd ${NEMBD} \
        --first-n-output 1 \
        --out "${OUT_DIR}/accuracy_eval_seq${ES}_arr120_first1.json" || echo "FAIL eval ${ES}"
done
