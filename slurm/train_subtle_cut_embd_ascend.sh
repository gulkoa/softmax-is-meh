#!/bin/bash
# Subtle-needle sweep with reduced n_embd (cut embeddings) and FIXED stj init.
# Env vars: ATTN, Q
...
OUT_DIR="results/subtle_needle_1layer_${TAG}_seq${SEQ}_embd${NEMBD}_h${NHEAD}_initfix_ascend"
mkdir -p "${OUT_DIR}"

echo "=== SUBTLE cut-embd: ${TAG} n_embd=${NEMBD} n_head=${NHEAD} ==="
