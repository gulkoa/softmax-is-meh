#!/bin/bash
# 1-HEAD toy model (Velickovic-style): train at small seq, eval far OOD.
...
OUT_DIR="results/subtle_needle_1head_${TAG}_seq${TRAIN_SEQ}_d${N_EMBD}_nope_ascend"
mkdir -p "${OUT_DIR}"

echo "=== subtle-needle 1-head toy ${TAG} seq=${TRAIN_SEQ} d=${N_EMBD} NoPE ==="
nvidia-smi
...
# Eval at aggressive extrapolation: 1×, 4×, 16×, 64×, 128× (cap 16384)
