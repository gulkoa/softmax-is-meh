...
COMMON="--task sorting --seq-len 4096 --max-arr-len 1024 --max-val 256 --lr 1e-4 --epochs 30 --batch-size 2 --train-samples 50000 --val-samples 5000 --resume"
...
python nanogpt/analyze.py --checkpoint "$OUTDIR/model.pt" --task sorting --attn softmax --out "$OUTDIR/analysis" --seq-len 4096 --max-arr-len 1024 --max-val 256
...
for Q in 1.0 2.0 8.0; do
    ...
    python nanogpt/analyze.py --checkpoint "$OUTDIR/model.pt" --task sorting --attn stieltjes --q "$Q" --out "$OUTDIR/analysis" --seq-len 4096 --max-arr-len 1024 --max-val 256
done
