#!/bin/bash
# Max retrieval sweep with reduced n_embd (cut embeddings) and FIXED stj init.
# Tests if softmax loses its embedding-amplification advantage at small n_embd.
# Env vars: ATTN, Q
...
OUT_DIR="results/max_1layer_${TAG}_seq${SEQ}_embd${NEMBD}_h${NHEAD}_initfix_ascend"
mkdir -p "${OUT_DIR}"

echo "=== MAX retrieval cut-embd: ${TAG} n_embd=${NEMBD} n_head=${NHEAD} ==="
