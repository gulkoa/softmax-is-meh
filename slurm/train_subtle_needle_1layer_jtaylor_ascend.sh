#!/bin/bash
# jtaylor-style stieltjes (init=1.1, no half-step, num_iter=3) - retrain at same config
# as fixedcap to enable apples-to-apples comparison.
# Env vars: Q
# Output dir: results/subtle_needle_1layer_stieltjes_jt_q<Q>_seq128_fixedcap_ascend/
...
TAG="stieltjes_jt_q${Q}"
EXTRA_TRAIN="--attn stieltjes_jt --q ${Q}"
EXTRA_EVAL="--attn stieltjes_jt --q ${Q}"

OUT_DIR="results/subtle_needle_1layer_${TAG}_seq${SEQ}_fixedcap_ascend"
mkdir -p "${OUT_DIR}"

echo "=== JTAYLOR stieltjes retrain: ${TAG} seq=${SEQ} ==="
