"""Custom handler for Hugging Face Inference Endpoints (base model).

Accepts BOTH formats:

1. OpenAI chat format ({"messages": [...], "max_tokens": ..}) —
   messages are folded into a plain-text dialogue transcript
   (User:/Assistant: lines); this is a BASE model with no trained chat
   template, so chat quality is completion-grade by design. Returns an
   OpenAI-shaped chat.completion response.

2. Classic HF format ({"inputs": "raw prompt", "parameters": {...}}) —
   plain completion, [{"generated_text": "..."}].
"""
import time
import uuid
...
class EndpointHandler:
    ...
    def _chat(self, data):
        # Base model: fold chat into a plain transcript. No special
        # tokens were trained; User:/Assistant: framing gives the
        # completion model a pattern to continue.
        lines = []
        ...
        # trim if the model continues the dialogue pattern
        text = text.split("\nUser:")[0].strip()
        return {
            "id": "chatcmpl-" + uuid.uuid4().hex[:24],
            "object": "chat.completion",
            "created": int(time.time()),
            "model": data.get("model", "stilt.1.1-355m-nope"),
            "choices": [{
                "index": 0,
